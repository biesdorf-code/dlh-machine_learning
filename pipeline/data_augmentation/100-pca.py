...
def pca_color(image, alphas):
    ...
    delta = tf.linalg.matvec(eig_vecs, scaled)  # a single faint tint
    out = tf.clip_by_value(img + delta, 0.0, 1.0)  # dog, slightly tinted
    return tf.cast(out * 255.0, tf.uint8)  # same tinted dog, for display
