#!/usr/bin/env python3
"""module for posterior: updated belief in each hypothesis given the data"""
import numpy as np
...
def marginal(x, n, P, Pr):
    ...
    return marginal_prob  # scalar


def posterior(x, n, P, Pr):
    """Calculates the posterior probability of each hypothesis"""
    # returns the numerator of Bayes rule
    intersections = intersection(x, n, P, Pr)

    # denominator
    marginal_prob = np.sum(intersections)

    # Bayes' theorem: P(P | data) = P(data AND P) / P(data)
    #                             = [ L(x | n, P) * Pr(P) ] / P(data)

    posteriors = intersections / marginal_prob

    return posteriors  # 1D nparray
